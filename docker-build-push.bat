@echo off
REM Build image
docker build -t influxdb-gateway ./http/influxdb
...
docker build -t python-monitoring ./http/python-monitoring
docker build -t grpc-ocr-server ./http/grpc-ocr 
docker build -t grpc-plate-server ./http/grpc-plate 
docker build -t grpc-vehicle-server ./httpgrpc-vehicle 
docker build -t ampq-minio-consumer ./message-broker/ampq-minio
docker build -t ampq-influxdb-consumer ./message-broker/ampq-influxdb
